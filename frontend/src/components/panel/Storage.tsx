...
import { getUploads } from "../../fetchers/getStorage"

export const Storage = () => {
    const { data, isLoading, error } = useQuery(['storageFiles'], getUploads)
    ...
    return (
        <div className="flex flex-col w-full">
            <header className="w-full p-8 border-b-2">
                <h1 className="text-xl font-bold uppercase">Your Uploads</h1>
            </header>
            <div className="p-8">
                {data.files.map((f: any) => {
                    return (
                        <div className="rounded-lg flex flex-row p-4 border-2 justify-between">
                            <img src={f.filepath} alt="image" className="max-w-8 max-h-8 rounded-full" />
